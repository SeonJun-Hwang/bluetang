...
import { TokenOption, PairChunk, Chunk, Content } from '~global/types';
...
class Chunker {
  ...
  private convertHTML(tag: string, content: Content): Content {
    ...
    const imgChanged = _.chunk(pairChangeed.split(PATTERNS.img.convert), IMG_HREF_SPLIT_LENGTH).map(imgMap).join('');
    return _.chunk(imgChanged.split(PATTERNS.a.convert), IMG_HREF_SPLIT_LENGTH).map(hrefMap).join('');
  }
}
