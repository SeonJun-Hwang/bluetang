...
export const createUserTable = async (email: string, title = 'title') => {
  ...
  db.collection('user').doc(email).set({ latestDoc: 'default' });
  return newUser;
};
...
export const updateData = async (email: string, id: string, title: string, data: string) => {
  const db = firestore();
  return db.collection(email).doc(id).update({
    timestamp: new Date(),
    title,
    data,
  });
...
export const updateTitle = async (email: string, id: string, title: string) => {
  ...
  return db.collection(email).doc(id).update({ title: safeTitle });
};
