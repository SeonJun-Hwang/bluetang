...
export const changeValidPairStrs = ( source: string, targets: Array<string>, validators: Array<RegExp>, pairs: Array<Array<string>> ) => {
  ...
  for ( let i = 0; i < targets.length; i++ ) {
    ...
    if ( splitedSource.length & 1 ) result = splitedSource
      .reduce( ( prev, cur, idx ) => prev + ( idx % 2 ? pairs[ i ][ 0 ] : pairs[ i ][ 1 ] ) + cur );
    else result = splitedSource
      .slice( 0, splitedSource.length - 1 )
      .reduce( ( prev, cur, idx ) => prev + ( idx % 2 ? pairs[ i ][ 0 ] : pairs[ i ][ 1 ] ) + cur ) +
      targets[ i ] +
      splitedSource[ splitedSource.length - 1 ];
  }
  return result;
};
...
export const base64ToStr = ( code: string ) => decodeURIComponent( atob( code ) );
